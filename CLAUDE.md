...
設計の全体像は [docs/design.md](docs/design.md)。ここには作業のルールだけを書く。

## コミット

- **`user.email` を上書きしない。** リポジトリ / グローバル設定の identity をそのまま使う（`kenshin.ml012@gmail.com`）。別アドレスでコミットすると GitHub 上で同名の別コントリビューターとして計上される
...
1. **生成に LLM を介在させない。** CLI の処理はスキーマ検証・テンプレート変数の置換・ファイル IO のみ。任意の式を評価しない
